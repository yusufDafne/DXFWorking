...
## `ScaleBar` KALDIRILDI (rev-17'de eklendi, rev-18'de geri alındı)

rev-17'de (DEV-025) burada bir `ScaleBar` (grafik ölçek çubuğu) sınıfı
vardı: `meta.scale`den türeyen, her pafta üzerine 5 segmentlik bir cetvel
çizen bir sunum ögesi. Kullanıcı bunu üretilen çıktıda görüp geri
bildirimde bulundu ("her pafta içerisinde ölçek gibi bir şey var, 0-5 arası
değerler ile birlikte, onu istemiyorum, kaldır") ve rev-18'de sınıf +
`nice_scale_length_m`/`format_scale_value` yardımcıları TAMAMEN
KALDIRILDI (`generate_dxf.py`/`preview.py`'deki çağrı yerleri, ilgili
selftest grupları ve bu bölüm dahil). Kuzey oku (`scripts/northarrow/`)
bundan etkilenmedi — bkz. o modülün `CLAUDE.md`'si. Bu modülün
"SORUMLULUK SINIRI"ndaki eski istisna notu da bu yüzden kaldırıldı; modül
artık yeniden SADECE pafta çerçevesi + başlık kutusu + taşma kontrolü +
kağıt boyutu hesaplarından sorumludur.
...
bir bilinen boşluktur. `Sheet` tarafı için bu modülde değişiklik yapan bir
ajan şunu doğrulamalı:
